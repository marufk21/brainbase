...
const kinds = ["client", "person", "project", "topic", "decision", "document"] as const;
type Kind = (typeof kinds)[number];
type Saved = { kind: Kind; id: string; label: string };
const inputClass =
...
function Editor({
  ...
  kind: Kind;
  initial?: Client | Person | Project | Topic | Decision | Document;
...
function ClientForm({ initial, onSave, onDelete }: FormProps<Client>) {
...
function PersonForm({ initial, onSave, onDelete }: FormProps<Person>) {
  const [name, setName] = useState(initial?.name ?? "");
...
function TopicForm({ initial, onSave, onDelete }: FormProps<Topic>) {
  const [name, setName] = useState(initial?.name ?? "");
...
function labelOf(item: Client | Person | Project | Topic | Decision | Document) {
